...
function cumlelereDonustur(cumleler, ayraç = ",") {
  let newcumleler = cumleler.map((cumle) => cumle.join(ayraç));
  return newcumleler;
}
...
function paragrafOlustur(dizi, callbackcumleKur, callbackcumlelereDonustur) {
  let yeniDizi = callbackcumleKur(dizi, " ");
  let paragraf = callbackcumlelereDonustur(
    yeniDizi[1],
    yeniDizi[2],
    yeniDizi[3],
    yeniDizi[4],
